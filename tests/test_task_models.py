...
def test_work_item_creation():
    item = WorkItem(
        id="abc123def456", project="paperweight",
        title="Fix the tests", description="Tests are flaky",
        source="manual",
    )
...
def test_work_item_with_all_fields():
    item = WorkItem(
        id="abc123def456", project="paperweight", template="issue-resolver",
        title="Fix bug PW-42", description="Login broken",
        source="linear", source_id="uuid-123",
        source_url="https://linear.app/pw/issue/PW-42",
        status=TaskStatus.RUNNING, session_id="session-abc",
