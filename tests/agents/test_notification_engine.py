...
def test_build_digest(engine, store):
    store.upsert_event(
        ...
        title="Bug fix", source_item_id="L1", timestamp="2026-03-16T10:00:00Z", priority="urgent",
    )
    store.upsert_event(
...
def test_build_digest_includes_source_summary(engine, store):
    store.upsert_event(
        project_id="p1", source="linear", event_type="issue_created",
        title="T1", source_item_id="L1", timestamp="2026-03-16T10:00:00Z", priority="high",
    )
    store.upsert_event(
        project_id="p1", source="linear", event_type="issue_created",
        title="T2", source_item_id="L2", timestamp="2026-03-16T10:01:00Z", priority="none",
    )
    digest = engine.build_digest("p1")
...
def test_check_urgent_alerts_includes_high(engine, store):
    store.upsert_event(
        project_id="p1", source="linear", event_type="issue_created",
        title="High prio", source_item_id="L1", timestamp="2026-03-16T10:00:00Z", priority="high",
    )
    alerts = engine.check_urgent_events("p1")
    assert len(alerts) == 1
...
def test_check_urgent_alerts_none(engine, store):
    store.upsert_event(
        project_id="p1", source="linear", event_type="issue_created",
        title="Minor fix", source_item_id="L1", timestamp="2026-03-16T10:00:00Z", priority="low",
    )
...
async def test_send_digest_disabled_rule(engine, store):
    store.create_notification_rule(
        project_id="p1", rule_type="digest", channel="slack",
        channel_target="dm", config={}, enabled=False,
    )
    await engine.send_digest("p1")
    engine.slack_notifier.send_text.assert_not_called()
...
async def test_send_urgent_alert(engine, store):
    ...
    event = {
        "id": "ev1", "title": "Critical!", "url": "https://example.com",
        "source_item_id": "L1", "priority": "urgent",
    }
