...
def register_task_routes(app: FastAPI, task_store: TaskStore) -> None:
    ...
    async def duplicate_work_item(item_id: str) -> Response | dict:
        new_item = task_store.duplicate(item_id)
        if new_item is None:
            return Response(status_code=404, content="Work item not found")
        return Response(
            status_code=201,
            content=new_item.model_dump_json(),
    ...
    async def rerun_work_item(item_id: str) -> Response | dict:
        ...
        if item is None:
            return Response(status_code=404, content="Work item not found")
        if not task_store.reset_for_rerun(item_id):
            return Response(status_code=409, content="Cannot rerun a running or pending task")
        updated = task_store.get(item_id)
        return updated.model_dump(mode="json")
